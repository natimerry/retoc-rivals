...
use crate::script_objects::{
    FPackageObjectIndex, FPackageObjectIndexType, FScriptObjectEntry, ZenScriptObjects,
};
use crate::ser::WriteExt;
use crate::zen::{
    EExportCommandType, EExportFilterFlags, EObjectFlags, FExportMapEntry, FExternalDependencyArc,
...
impl<'a> FZenPackageContext<'a> {
    ...
    fn find_script_object(
        &self,
        script_object_index: FPackageObjectIndex,
    ) -> anyhow::Result<FScriptObjectEntry> {
        ...
        let script_objects: &ZenScriptObjects =
            &script_objects_lock.as_ref().unwrap().script_objects;
        let script_object_entry = script_objects
            .script_object_lookup
            .get(&script_object_index)
            .ok_or_else(|| {
                anyhow!(
                    "Failed to find script object with ID {}",
                    script_object_index
                )
            })?;
        Ok(*script_object_entry)
    }
    fn resolve_script_object_name(
        &self,
        script_object_name: FMappedName,
        ...
        Ok(script_objects
            .global_name_map
            .get(script_object_name)
            .to_string())
    }
    fn is_script_object_class(&self, script_object_index: FPackageObjectIndex) -> bool {
        let script_objects_lock = self.get_script_objects().unwrap();
        let script_objects_resolved_as_classes = &script_objects_lock
            .as_ref()
            .unwrap()
            .script_objects_resolved_as_classes;
    ...
    fn lookup(&self, package_id: FPackageId) -> anyhow::Result<Arc<FZenPackageHeader>> {
        ...
        if package_store_entry_ref.is_none() {
            ...
            write_lock.packages_failed_load.insert(package_id);
            return Err(anyhow!(
                "Failed to find Package Store Entry for Package Id {}",
                package_id
            ));
        }
        ...
        let container_header_version = self
            .store_access
            .container_header_version()
            .ok_or_else(|| anyhow!("Failed to retrieve container header version"))?;

        let zen_package_header = FZenPackageHeader::deserialize(
...
fn resolve_script_import(
...
) -> anyhow::Result<ResolvedZenImport> {
    ...
    if script_object.outer_index.kind() != FPackageObjectIndexType::ScriptImport {
        bail!(
            "Outer script object {} for import {} is not a script import",
            script_object.outer_index,
            import
        );
    }
    ...
    let is_cdo_object =
        resolved_outer_import.outer.is_none() && object_name.starts_with("Default__");
    if is_cdo_object && !script_object.cdo_class_index.is_null() {
        ...
        let resolved_class_package = resolved_class
            .outer
            .ok_or_else(|| anyhow!("Failed to resolve CDO class package"))?;
        if resolved_class_package.outer.is_some() {
            bail!(
                "Resolved CDO class outer was not a UPackage for class {} of CDO {}",
                script_object.cdo_class_index,
                import
            );
        }
        return Ok(ResolvedZenImport {
...
fn resolve_package_import_internal_new(
    ...
    let package_import = import
        .package_import()
        .ok_or_else(|| anyhow!("Failed to resolve import {} as package import", import))?;
    ...
    let package_id: FPackageId =
        package_header.imported_packages[package_import.imported_package_index as usize];

    // Make sure the hash index points to a valid hash in the package header
    if package_import.imported_public_export_hash_index as usize
    ...
    resolve_package_export_internal(
        package_cache,
        resolved_import_package.as_ref(),
        imported_export,
    )
}
fn resolve_package_import_internal_legacy(
    package_cache: &FZenPackageContext,
    package_header: &FZenPackageHeader,
    import: FPackageObjectIndex,
) -> anyhow::Result<ResolvedZenImport> {
    ...
    for imported_package_id in &package_header.imported_packages {
        ...
        let potential_imported_export = resolved_import_package
            .export_map
            .iter()
            .find(|x| x.legacy_global_import_index() == import);
        ...
        if let Some(imported_export) = potential_imported_export {
            return resolve_package_export_internal(
                package_cache,
                resolved_import_package.as_ref(),
                imported_export,
            );
...
fn resolve_package_export_internal(
    package_cache: &FZenPackageContext,
    package_header: &FZenPackageHeader,
    export: &FExportMapEntry,
) -> anyhow::Result<ResolvedZenImport> {
    ...
    let resolved_export_class = resolve_generic_zen_import_import(
        package_cache,
        package_header,
        export.class_index,
        false,
    ...
    let resolved_export_outer =
        resolve_generic_zen_import_import(package_cache, package_header, export.outer_index, true)?;

    // Resolved class of the export must be exactly 1 level deep, so it must have an outer that is a package
    if resolved_export_class.outer.is_none()
        || resolved_export_class
            .outer
            .as_ref()
            .unwrap()
            .outer
...
fn resolve_builder_package_as_import(
    package_header: &FZenPackageHeader,
) -> anyhow::Result<ResolvedZenImport> {
    Ok(ResolvedZenImport {
        class_package: CORE_OBJECT_PACKAGE_NAME.to_string(),
...
fn resolve_generic_zen_import_import(
    package_cache: &FZenPackageContext,
    package_header: &FZenPackageHeader,
    import: FPackageObjectIndex,
    resolve_null_as_this_package: bool,
) -> anyhow::Result<ResolvedZenImport> {
    match import.kind() {
        FPackageObjectIndexType::ScriptImport => resolve_script_import(package_cache, import),
        FPackageObjectIndexType::PackageImport => {
            resolve_package_import(package_cache, package_header, import)
        }
        FPackageObjectIndexType::Export => {
            resolve_package_export(package_cache, package_header, import)
        }
        FPackageObjectIndexType::Null => {
            if resolve_null_as_this_package {
                ...
                bail!(
                    "Encountered Null object while parsing the import map entry {} of package {}",
                    import,
                    package_header.name_map.get(package_header.summary.name)
                )
            }
        }
    }
...
fn begin_build_summary(builder: &mut LegacyAssetBuilder) -> anyhow::Result<()> {
    ...
    if builder.zen_package.is_unversioned
        && builder
            .package_context
            .fallback_package_file_version
            .is_none()
        && builder.package_context.log.allow_stdout()
    {
...
fn copy_package_sections(builder: &mut LegacyAssetBuilder) -> anyhow::Result<()> {
    ...
    builder.legacy_package.name_map =
        FPackageNameMap::create_from_names(builder.zen_package.name_map.copy_raw_names());
    // Current number of names is the minimum that should be kept
    builder
        .legacy_package
        .summary
        .names_referenced_from_export_data_count =
        builder.legacy_package.name_map.num_names() as i32;
...
fn resolve_local_package_object(
    builder: &mut LegacyAssetBuilder,
    package_object: FPackageObjectIndex,
    ...
    let resolved_import = resolve_generic_zen_import_import(
        builder.package_context,
        builder.zen_package.as_ref(),
        package_object,
        false,
    )?;
    ...
    builder
        .zen_import_lookup
        .insert(package_object, import_map_index);
    Ok(import_map_index)
}
fn find_or_add_resolved_import(
    builder: &mut LegacyAssetBuilder,
    import: &ResolvedZenImport,
) -> FPackageIndex {
    ...
    let new_import_index =
        FPackageIndex::create_import(builder.legacy_package.imports.len() as u32);
    ...
    builder
        .resolved_import_lookup
        .insert(import.clone(), new_import_index);
...
fn build_import_map(builder: &mut LegacyAssetBuilder) -> anyhow::Result<()> {
    for import_index in 0..builder.zen_package.import_map.len() {
        ...
        let import_map_index = if result_import_map_index.is_err() {
            ...
            builder
                .zen_import_lookup
                .insert(import_object_index, import_map_index);
            import_map_index
        } else {
        ...
        if !import_map_index.is_import() {
            bail!(
                "Import map package object index {} did not resolve into an import for package {}",
                import_object_index,
                builder.zen_package.package_name()
            );
        }
        ...
        builder
            ...
            .insert(import_index, import_map_index.to_import_index() as usize);
    }
    Ok(())
}
fn build_export_map(builder: &mut LegacyAssetBuilder) -> anyhow::Result<()> {
    builder
        .legacy_package
        .exports
        .reserve(builder.zen_package.export_map.len());
    ...
    for export_index in 0..builder.zen_package.export_map.len() {
        let zen_export: FExportMapEntry =
            builder.zen_package.as_ref().export_map[export_index].clone();
        ...
        let export_name_string = builder
            .zen_package
            .name_map
            .get(zen_export.object_name)
            .to_string();
        ...
        let serial_offset: i64 = if !builder.needs_to_rebuild_exports_data {
            zen_export.cooked_serial_offset as i64
        } else {
            -1
        };
        ...
        let asset_object_flags: u32 = (EObjectFlags::Public as u32)
            | (EObjectFlags::Standalone as u32)
            | (EObjectFlags::Transactional as u32);
        let is_asset = zen_export.outer_index.is_null()
            && (object_flags & asset_object_flags) == asset_object_flags;
        ...
        let generate_public_hash = builder.zen_package.container_header_version
            >= EIoContainerHeaderVersion::LocalizedPackages
            && (object_flags & (EObjectFlags::Public as u32)) == 0
            && zen_export.is_public_export();
...
fn resolve_export_dependencies_internal_dependency_bundles(
    builder: &mut LegacyAssetBuilder,
    export_dependencies: &mut [FStandaloneExportDependencies],
) {
    for (export_index, bundle_header) in builder
        .zen_package
        ...
        .enumerate()
    {
        ...
        let remap_zen_package_index = |x: FPackageIndex| -> FPackageIndex {
            if x.is_import() {
                FPackageIndex::create_import(
                    *builder
                        .original_import_order
                        .get(&(x.to_import_index() as usize))
                        .unwrap() as u32,
                )
            } else {
                x
        ...
        let last_create_before_create_index =
            first_dependency_index + bundle_header.create_before_create_dependencies as usize;
        dependencies.create_before_create = builder.zen_package.dependency_bundle_entries
            [first_dependency_index..last_create_before_create_index]
            .iter()
        ...
        let last_serialize_before_create_index = last_create_before_create_index
            + bundle_header.serialize_before_create_dependencies as usize;
        dependencies.serialize_before_create = builder.zen_package.dependency_bundle_entries
            [last_create_before_create_index..last_serialize_before_create_index]
            .iter()
            .map(|x| x.local_import_or_export_index)
            .map(remap_zen_package_index)
            .collect();
        ...
        let last_create_before_serialize_index = last_serialize_before_create_index
            + bundle_header.create_before_serialize_dependencies as usize;
        dependencies.create_before_serialize = builder.zen_package.dependency_bundle_entries
            [last_serialize_before_create_index..last_create_before_serialize_index]
            .iter()
            .map(|x| x.local_import_or_export_index)
            .map(remap_zen_package_index)
            .collect();
        ...
        let last_serialize_before_serialize_index = last_create_before_serialize_index
            + bundle_header.serialize_before_serialize_dependencies as usize;
        dependencies.serialize_before_serialize = builder.zen_package.dependency_bundle_entries
            [last_create_before_serialize_index..last_serialize_before_serialize_index]
            .iter()
            .map(|x| x.local_import_or_export_index)
            .map(remap_zen_package_index)
            .collect();
...
fn resolve_export_dependencies_internal_dependency_arcs(
    builder: &mut LegacyAssetBuilder,
    export_dependencies: &mut [FStandaloneExportDependencies],
) -> anyhow::Result<()> {
    ...
    let mut add_export_dependency = |from_index: FPackageIndex,
                                     to_export_index: usize,
                                     from_type: EExportCommandType,
                                     to_type: EExportCommandType|
     -> anyhow::Result<()> {
        ...
        // for "to export" to be in Created state, it needs "from export" to be in Created (or Serialized) state
            ...
            if from_type == EExportCommandType::Create {
                ...
                export_dependencies[to_export_index]
    ...
    for bundle_header_index in 0..builder.zen_package.export_bundle_headers.len() {
        ...
        for i in 1..bundle_header.entry_count {
            ...
            let from_bundle_entry =
                builder.zen_package.export_bundle_entries[from_bundle_entry_index as usize];

            let to_bundle_entry_index = bundle_header.first_entry_index + i;
            let to_bundle_entry =
                builder.zen_package.export_bundle_entries[to_bundle_entry_index as usize];
    ...
    for internal_arc in internal_dependency_arcs {
        let from_export_bundle = builder.zen_package.export_bundle_headers
            [internal_arc.from_export_bundle_index as usize];
        let from_export_bundle_last_element_index =
            from_export_bundle.first_entry_index + from_export_bundle.entry_count - 1;
        let from_export_bundle_last_element = builder.zen_package.export_bundle_entries
            [from_export_bundle_last_element_index as usize];

        let from_export_index =
            FPackageIndex::create_export(from_export_bundle_last_element.local_export_index);
        ...
        let to_export_bundle =
            builder.zen_package.export_bundle_headers[internal_arc.to_export_bundle_index as usize];
        let to_export_bundle_first_element =
            builder.zen_package.export_bundle_entries[to_export_bundle.first_entry_index as usize];
    ...
    let all_external_arcs: Vec<FExternalDependencyArc> = builder
        .zen_package
        .external_package_dependencies
        .iter()
        .flat_map(|x| x.external_dependency_arcs.clone())
        .collect();
    for external_arc in all_external_arcs {
        // Try to find the first Serialize element in the To bundle and not just the first element, since that works better for preventing circular dependencies
        let to_export_bundle =
            builder.zen_package.export_bundle_headers[external_arc.to_export_bundle_index as usize];
        let to_export_bundle_entry =
            builder.zen_package.export_bundle_entries[to_export_bundle.first_entry_index as usize];
        ...
        let from_import_index_raw = *builder
            .original_import_order
            .get(&from_original_import_index)
            .unwrap() as u32;
    ...
    if builder.zen_package.container_header_version <= EIoContainerHeaderVersion::Initial {
        for external_package_dependency in builder.zen_package.external_package_dependencies.clone()
        {
            ...
            for bundle_to_bundle_dependency_arc in
                &external_package_dependency.legacy_dependency_arcs
            {
                ...
                let from_bundle_index =
                    if bundle_to_bundle_dependency_arc.from_export_bundle_index == -1 {
                ...
                let from_export_bundle =
                    resolved_import_package.export_bundle_headers[from_bundle_index];
                let from_export_bundle_last_element_index = (from_export_bundle.first_entry_index
                    + from_export_bundle.entry_count
                    - 1) as usize;
                let from_export_bundle_entry = resolved_import_package.export_bundle_entries
                    [from_export_bundle_last_element_index];

                // Create fully resolved zen import from that export
                let resolved_from_export_entry = resolved_import_package.export_map
                    [from_export_bundle_entry.local_export_index as usize]
                    .clone();
                let resolved_from_import_result = resolve_package_export_internal(
                    builder.package_context,
                    &resolved_import_package,
                    &resolved_from_export_entry,
                );
                ...
                if resolved_from_import_result.is_err() {
                    let resolution_error_message =
                        resolved_from_import_result.unwrap_err().to_string();
                ...
                let from_import_index =
                    find_or_add_resolved_import(builder, &resolved_from_import_result?);
                let from_command_type = from_export_bundle_entry.command_type;

                // Resolve the first element of the to export bundle
                let to_export_bundle = builder.zen_package.export_bundle_headers
                    [bundle_to_bundle_dependency_arc.to_export_bundle_index as usize];
                let to_export_bundle_entry = builder.zen_package.export_bundle_entries
                    [to_export_bundle.first_entry_index as usize];
...
fn apply_standalone_dependencies_to_package(
    builder: &mut LegacyAssetBuilder,
    export_dependencies: &mut [FStandaloneExportDependencies],
) {
    for (export_index, export_object) in builder.legacy_package.exports.iter_mut().enumerate() {
        ...
        if !export_object.outer_index.is_null()
            && !dependencies
                .create_before_create
                .contains(&export_object.outer_index)
        {
            dependencies
                .create_before_create
                .push(export_object.outer_index);
        }
        if !export_object.super_index.is_null()
            && !dependencies
                .create_before_create
                .contains(&export_object.super_index)
        {
            dependencies
                .serialize_before_serialize
                .push(export_object.super_index);
        }
        // Ensure that we have class and archetype as serialize before create dependencies
        if !export_object.class_index.is_null()
            && !dependencies
                .serialize_before_create
                .contains(&export_object.class_index)
        {
            dependencies
                .serialize_before_create
                .push(export_object.class_index);
        }
        if !export_object.template_index.is_null()
            && !dependencies
                .serialize_before_create
                .contains(&export_object.template_index)
        {
            dependencies
                .serialize_before_create
                .push(export_object.template_index);
        }

        // If we have no dependencies altogether, do not write first dependency import on the export
        if dependencies.create_before_create.is_empty()
            && dependencies.serialize_before_create.is_empty()
            && dependencies.create_before_serialize.is_empty()
            && dependencies.serialize_before_serialize.is_empty()
        {
            continue;
        }

        // Set the index of the preload dependencies start, and the numbers of each dependency
        export_object.first_export_dependency_index =
            builder.legacy_package.preload_dependencies.len() as i32;
        export_object.serialize_before_serialize_dependencies =
            dependencies.serialize_before_serialize.len() as i32;
        export_object.create_before_serialize_dependencies =
            dependencies.create_before_serialize.len() as i32;
        export_object.serialize_before_create_dependencies =
            dependencies.serialize_before_create.len() as i32;
        export_object.create_before_create_dependencies =
        ...
        builder
            ...
            .append(&mut dependencies.serialize_before_serialize);
        builder
            .legacy_package
            .preload_dependencies
            .append(&mut dependencies.create_before_serialize);
        builder
            .legacy_package
            .preload_dependencies
            .append(&mut dependencies.serialize_before_create);
        builder
            .legacy_package
            .preload_dependencies
            .append(&mut dependencies.create_before_create);
    }
}

fn resolve_export_dependencies(builder: &mut LegacyAssetBuilder) -> anyhow::Result<()> {
    // Create standalone dependency objects first
    let mut export_dependencies: Vec<FStandaloneExportDependencies> =
        Vec::with_capacity(builder.legacy_package.exports.len());
...
fn resolve_prestream_package_imports(builder: &mut LegacyAssetBuilder) -> anyhow::Result<()> {
    ...
    let prestream_package_ids: Vec<FPackageId> = builder
        .zen_package
        .imported_packages
        .iter()
        .cloned()
    ...
    for prestream_package_id in &prestream_package_ids {
        ...
        if resolved_prestream_package.is_err() {
            log!(
                builder.package_context.log,
                "Failed to resolve a pre-stream request to the package {} from package {}",
                prestream_package_id.clone(),
                builder.zen_package.package_name()
            );
            continue;
        }
        ...
        let class_package = builder
            .legacy_package
            .name_map
            .store(CORE_OBJECT_PACKAGE_NAME);
        let class_name = builder
            .legacy_package
            .name_map
            .store(PRESTREAM_PACKAGE_CLASS_NAME);
        let object_name = builder
            .legacy_package
            .name_map
            .store(&resolved_package_name);

        // Log that we have resolved the pre-stream request successfully
        if builder.package_context.log.verbose_enabled() {
            log!(
                builder.package_context.log,
                "Resolved pre-stream package request {} from package {}",
                resolved_package_name.clone(),
...
fn create_unknown_object_import_map_entry(
    builder: &mut LegacyAssetBuilder,
...
fn rebuild_asset_export_data_internal(
    builder: &LegacyAssetBuilder,
    raw_exports_data: &[u8],
) -> anyhow::Result<Vec<u8>> {
    // Calculate the total size of the exports blob
    let mut total_exports_serial_size = 0;
    for export_index in 0..builder.legacy_package.exports.len() {
        total_exports_serial_size +=
        ...
        for i in 0..export_bundle.entry_count {
            ...
            if export_bundle_entry.command_type == EExportCommandType::Serialize {
                ...
                let export_serial_size =
                    builder.legacy_package.exports[export_index].serial_size as usize;
                let export_target_serial_offset =
                    builder.legacy_package.exports[export_index].serial_offset as u64;
                ...
                exports_data_writer.write_all(
                    &raw_exports_data[export_data_start_offset..export_data_end_offset],
                )?;
...
fn serialize_asset(builder: &LegacyAssetBuilder) -> anyhow::Result<FSerializedAssetBundle> {
    ...
    FLegacyPackageHeader::serialize(
        &builder.legacy_package,
        &mut asset_cursor,
        Some(builder.zen_package.summary.cooked_header_size as usize),
        builder.package_context.log,
    )?;
...
fn write_asset(
    ...
    file_writer.write_file(
        out_asset_path.to_string(),
        true,
        serialized_asset.asset_file_buffer,
    )?;
    // Write the exports file
    let export_file_path = out_asset_path.with_extension("uexp");
    file_writer.write_file(
        export_file_path.to_string(),
        true,
        serialized_asset.exports_file_buffer,
    )?;
    ...
    if let Some(optional_bulk_data_buffer) = serialized_asset.optional_bulk_data_buffer {
        ...
        file_writer.write_file(
            optional_bulk_data_file_path.to_string(),
            true,
            optional_bulk_data_buffer,
        )?;
    }
    ...
    if let Some(memory_mapped_bulk_data_buffer) = serialized_asset.memory_mapped_bulk_data_buffer {
        ...
        file_writer.write_file(
            memory_mapped_bulk_data_file_path.to_string(),
            false,
            memory_mapped_bulk_data_buffer,
        )?;
    }
