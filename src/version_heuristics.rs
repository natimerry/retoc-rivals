...
use crate::zen::{
    EUnrealEngineObjectUE4Version, EUnrealEngineObjectUE5Version, EZenPackageVersion,
    FPackageFileVersion, FZenPackageSummary, FZenPackageVersioningInfo,
};
...
pub(crate) fn heuristic_zen_version_from_package_file_version(
    package_file_version: FPackageFileVersion,
    container_header_version: EIoContainerHeaderVersion,
) -> EZenPackageVersion {
    // UE 4.27, 5.0 and 5.1: initial
    if package_file_version.file_version_ue5
    ...
    } else if package_file_version.file_version_ue5
...
pub(crate) fn heuristic_zen_package_version(
    optional_package_version: Option<FPackageFileVersion>,
    container_version: EIoStoreTocVersion,
    container_header_version: EIoContainerHeaderVersion,
    has_bulk_data: bool,
) -> anyhow::Result<FZenPackageVersioningInfo> {
...
pub(crate) fn heuristic_package_version_from_legacy_package<S: Read + Seek>(
    s: &mut S,
    package_version_fallback: Option<FPackageFileVersion>,
) -> anyhow::Result<FPackageFileVersion> {
    let stream_start_position = s.stream_position()?;

    // Read the members that are independent on the package version
    let (versioning_info, names, _, _, package_flags) =
        FLegacyPackageFileSummary::deserialize_summary_minimal_version_independent(s)?;
    ...
    let package_flags_cooked_versioned =
    ...
    let package_versions_to_try: Vec<FPackageFileVersion> = vec![
        ...
        FPackageFileVersion::create_ue5(
    ...
    for package_version in package_versions_to_try {
        ...
        let package_summary_or_error: anyhow::Result<FLegacyPackageFileSummary> =
            FLegacyPackageFileSummary::deserialize(&mut read_cursor, Some(package_version));
        ...
        let is_standard_serialization_order = package_summary.names.offset
            < package_summary.imports.offset
        ...
        if package_summary.imports.count > 0 {
            ...
            let imports_start_offset =
                stream_start_position + package_summary.imports.offset as u64;
            let combined_imports_length =
                (package_summary.exports.offset - package_summary.imports.offset) as u64;
            ...
            let first_import: anyhow::Result<FObjectImport> =
                FObjectImport::deserialize(&mut first_import_cursor, &package_summary);
        ...
        if package_summary.exports.count > 0 {
            ...
            let exports_start_offset =
                stream_start_position + package_summary.exports.offset as u64;
            let combined_exports_length =
                (package_summary.depends_offset - package_summary.exports.offset) as u64;
            ...
            let first_export: anyhow::Result<FObjectExport> =
                FObjectExport::deserialize(&mut first_export_cursor, &package_summary);
...
pub(crate) fn heuristic_can_downgrade_import_preload_dependency_generic(
    package: &FLegacyPackageHeader,
    from_import_index: usize,
) -> bool {
    let object_import = package.imports[from_import_index].clone();
    let class_package = package
        .name_map
        .get(object_import.class_package)
        .to_string();
