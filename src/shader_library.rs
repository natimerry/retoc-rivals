...
impl FIoStoreShaderCodeEntry {
    ...
    const SHADER_GROUP_INDEX_SHIFT: u64 =
        Self::SHADER_FREQUENCY_SHIFT + Self::SHADER_FREQUENCY_BITS;
    const SHADER_GROUP_INDEX_BITS: u64 = 30;
    const SHADER_GROUP_INDEX_MASK: u64 = (1 << Self::SHADER_GROUP_INDEX_BITS) - 1;
    const SHADER_UNCOMPRESSED_OFFSET_IN_GROUP_SHIFT: u64 =
        Self::SHADER_GROUP_INDEX_SHIFT + Self::SHADER_GROUP_INDEX_BITS;
    const SHADER_UNCOMPRESSED_OFFSET_IN_GROUP_BITS: u64 = 30;
    const SHADER_UNCOMPRESSED_OFFSET_IN_GROUP_MASK: u64 =
        (1 << Self::SHADER_UNCOMPRESSED_OFFSET_IN_GROUP_BITS) - 1;
    ...
    fn shader_uncompressed_offset_in_group(self) -> usize {
        ((self.packed >> Self::SHADER_UNCOMPRESSED_OFFSET_IN_GROUP_SHIFT)
            & Self::SHADER_UNCOMPRESSED_OFFSET_IN_GROUP_MASK) as usize
    }
    fn new(
        shader_group_index: usize,
        shader_uncompressed_offset_in_group: usize,
        shader_frequency: u8,
    ) -> Self {
        let packed: u64 = ((shader_frequency as u64 & Self::SHADER_FREQUENCY_MASK)
            << Self::SHADER_FREQUENCY_SHIFT)
            | ((shader_group_index as u64 & Self::SHADER_GROUP_INDEX_MASK)
                << Self::SHADER_GROUP_INDEX_SHIFT)
            | ((shader_uncompressed_offset_in_group as u64
                & Self::SHADER_UNCOMPRESSED_OFFSET_IN_GROUP_MASK)
                << Self::SHADER_UNCOMPRESSED_OFFSET_IN_GROUP_SHIFT);
        Self { packed }
    }
}
impl Debug for FIoStoreShaderCodeEntry {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("FIoStoreShaderCodeEntry")
            ...
            .field(
                "shader_uncompressed_offset_in_group",
                &self.shader_uncompressed_offset_in_group(),
            )
            .finish()
    }
}
...
impl FIoStoreShaderCodeArchiveHeader {
    fn deserialize<S: Read>(
        s: &mut S,
        _version: EIoStoreShaderLibraryVersion,
    ) -> anyhow::Result<Self> {
...
fn determine_likely_compression_method_for_shader_code(
    shader_group_data: &[u8],
) -> CompressionMethod {
    ...
    } else if shader_group_data
        .first()
        .is_some_and(|f| [0x78, 0x58].contains(f))
    {
        CompressionMethod::Zlib
    } else if shader_group_data
        .first()
        .is_some_and(|f| [0x8C].contains(f))
    {
...
fn decompress_shader_code_with_method(
    shader_group_data: &[u8],
    ...
    decompress(
        compression_method,
        shader_group_data,
        &mut result_uncompressed_data,
    )?;
    Ok(result_uncompressed_data)
}

fn decompress_shader_code(
    shader_group_data: &[u8],
    compression_method: &mut Option<CompressionMethod>,
    uncompressed_size: usize,
) -> anyhow::Result<Vec<u8>> {
    if compression_method.is_none() {
        // compression method unknown so try to determine it
        let likely_compression =
            determine_likely_compression_method_for_shader_code(shader_group_data);
        ...
        for compression in compression_to_try {
            let result = decompress_shader_code_with_method(
                shader_group_data,
                compression,
                uncompressed_size,
            );
        ...
        decompress_shader_code_with_method(
            shader_group_data,
...
fn compress_shader(
    ...
    compress(
        compression_method,
        shader_data,
        Cursor::new(&mut compression_buffer),
    )?;
...
impl IoStoreShaderCodeArchive {
    ...
    pub(crate) fn read(
    ...
    ) -> anyhow::Result<IoStoreShaderCodeArchive> {
        ...
        let zen_shader_library_version = EIoStoreShaderLibraryVersion::from_repr(
            zen_shader_library_version_raw,
        )
        .ok_or_else(|| {
            anyhow!(
                "Unknown shader library version: {}",
                zen_shader_library_version_raw
            )
        })?;
        let shader_library_header = FIoStoreShaderCodeArchiveHeader::deserialize(
            &mut shader_library_reader,
            zen_shader_library_version,
        )?;
        ...
        let mut decompressed_shaders: Vec<Vec<u8>> =
            vec![Vec::new(); shader_library_header.shader_entries.len()];
        let mut total_shader_code_size: usize = 0;

        for shader_group_index in 0..shader_library_header.shader_group_entries.len() {
            let shader_group_chunk_id =
                shader_library_header.shader_group_chunk_ids[shader_group_index];
            ...
            if shader_group_entry.compressed_size != shader_group_entry.uncompressed_size {
                ...
                shader_group_data = decompress_shader_code(
                    &shader_group_data,
            ...
            let mut shader_id_and_offset: Vec<(usize, usize)> =
                Vec::with_capacity(shader_group_entry.num_shaders as usize);
            for i in 0..shader_group_entry.num_shaders {
                ...
                let shader_index =
                    shader_library_header.shader_indices[shader_indices_index] as usize;
            ...
            for i in 0..(shader_id_and_offset.len() - 1) {
                ...
                decompressed_shaders[shader_index] =
                    shader_group_data[shader_start_offset..shader_end_offset].to_vec();
...
fn layout_write_shader_code(
    shader_library: &IoStoreShaderCodeArchive,
    compress_shaders: bool,
) -> anyhow::Result<WriteShaderCodeResult> {
    ...
    let mut shader_map_file_regions: Vec<(i64, usize)> =
        vec![(-1, 0); shader_library.header.shader_map_entries.len()];
    ...
    pariter::scope(|s| -> anyhow::Result<()> {
        s.spawn(|_| -> anyhow::Result<()> {
            ...
            for shader_map_index in 0..shader_library.header.shader_map_entries.len() {
                ...
                for i in 0..shader_map_entry.num_shaders {
                    let shader_indices_index =
                        (shader_map_entry.shader_indices_offset + i) as usize;
                    let shader_index =
                        shader_library.header.shader_indices[shader_indices_index] as usize;
            ...
            .parallel_map_scoped(s, |message| {
                match message {
                    Message::Compress {
                        ...
                        let shader_uncompressed_size =
                            shader_library.shaders_code[shader_index].len();
                        ...
                        if compress_shaders && shader_library.compression_method.is_some() {
                            let shader_compressed_data = compress_shader(
                                shader_uncompressed_data,
                                shader_library.compression_method.unwrap(),
                            )?;
            ...
            .try_for_each(|message: anyhow::Result<Message>| -> anyhow::Result<()> {
                ...
                match message? {
                    ...
                    Message::EndMap { shader_map_index } => {
                        ...
                        let shader_map_total_size =
                            (shader_map_end_offset - shader_map_start_offset) as usize;
                        shader_map_file_regions[shader_map_index] =
                            (shader_map_start_offset as i64, shader_map_total_size);
                    }
                    Message::Write {
                        shader_index,
                        data,
                        ...
                        shader_file_regions[shader_index] =
                            (stream_position as i64, data.len(), unique);
    ...
    if let Some(missing_index) = shader_file_regions.iter().position(|s| s.0 < 0) {
        bail!(
            "Did not write shader code at index {} into the shader code archive",
            missing_index
        );
    }
...
pub(crate) fn get_shader_asset_info_filename_from_library_filename(
    shader_library_filename: &str,
) -> anyhow::Result<String> {
    let library_name_without_extension = UEPath::new(shader_library_filename)
        .file_stem()
        .ok_or_else(|| anyhow!("Failed to retrieve filename from path"))?;
    let prefix_separator_index = library_name_without_extension.find('-').ok_or_else(|| {
        anyhow!(
            "Invalid shader library filename, does not have a library name and format separator"
        )
    })?;
...
fn build_shader_asset_metadata_from_io_store_packages(
    store_access: &dyn IoStoreTrait,
    shader_map_hashes: &HashSet<FSHAHash>,
    log: &Log,
) -> (usize, ShaderAssetInfoFileRoot) {
    ...
    for package_info in store_access.packages() {
        ...
        if let Some(package_store_entry) = package_info.container().package_store_entry(package_id)
        {
            ...
            let referenced_shader_map_hashes: Vec<FSHAHash> = package_store_entry
                .shader_map_hashes
                .iter()
                .filter(|x| shader_map_hashes.contains(x))
                .cloned()
                .collect();
            ...
            let package_data_buffer = store_access.read(FIoChunkId::from_package_id(
                package_id,
                0,
            ...
            let package_header = FZenPackageHeader::get_package_name(
                &mut Cursor::new(&package_data_buffer.unwrap()),
                container_header_version.unwrap(),
            );
            ...
            for shader_map_hash in referenced_shader_map_hashes {
                shader_map_hash_to_package_names
                    .entry(shader_map_hash)
                    .or_default()
    ...
    let mut referenced_shader_map_hashes: Vec<FSHAHash> =
        shader_map_hash_to_package_names.keys().cloned().collect();
    ...
    for shader_map_hash in &referenced_shader_map_hashes {
        shader_asset_info
            .shader_code_to_assets
            .push(ShaderMapToPackageNameListEntry {
                shader_map_hash: *shader_map_hash,
                package_names: shader_map_hash_to_package_names
                    .get(shader_map_hash)
                    .unwrap()
                    .clone(),
            })
...
pub(crate) fn rebuild_shader_library_from_io_store(
    store_access: &dyn IoStoreTrait,
    library_chunk_id: FIoChunkId,
    log: &Log,
    compress_shaders: bool,
    ...
    let library_name = store_access
        ...
        .ok_or_else(|| {
            anyhow!(
                "Failed to retrieve IoStore shader library name for shader library chunk {:?}",
                library_chunk_id
            )
        })?;
    ...
    shader_library
        .shader_entries
        .reserve(io_store_shader_library.header.shader_entries.len());
    for shader_index in 0..io_store_shader_library.header.shader_entries.len() {
        let shader_frequency =
            io_store_shader_library.header.shader_entries[shader_index].shader_frequency();
        let uncompressed_shader_code_size =
            io_store_shader_library.shaders_code[shader_index].len();
        let (shader_code_offset, compressed_shader_code_size, _) =
            shader_code.shader_regions[shader_index];
    ...
    shader_library
        .shader_map_entries
        .reserve(io_store_shader_library.header.shader_map_entries.len());
    for shader_map_index in 0..io_store_shader_library.header.shader_map_entries.len() {
        ...
        let (shader_map_start_offset, shader_map_size) =
            shader_code.shader_map_regions[shader_map_index];
        ...
        for i in 0..shader_map_entry.num_shaders {
            let shader_indices_index = (shader_map_entry.shader_indices_offset + i) as usize;
            let shader_index =
                io_store_shader_library.header.shader_indices[shader_indices_index] as usize;
            let (shader_start_offset, shader_compressed_size, is_shader_unique) =
                shader_code.shader_regions[shader_index];
    ...
    let contained_shader_map_hashes: HashSet<FSHAHash> =
        shader_library.shader_map_hashes.iter().cloned().collect();
    let (total_package_references, shader_asset_info) =
        build_shader_asset_metadata_from_io_store_packages(
            store_access,
            &contained_shader_map_hashes,
            log,
        );
    ...
    if log.allow_stdout() {
        let compression_ratio = f64::round(
            (io_store_shader_library.total_shader_code_size as f64
                / shader_code.shader_code_buffer.len() as f64)
                * 100.0f64,
        ) as i64;
    ...
    Ok((
        result_shader_library_buffer,
        result_shader_asset_metadata_buffer,
    ))
}

fn is_raytracing_shader_frequency(shader_frequency: u8) -> bool {
    shader_frequency == EShaderFrequency::RayGen as u8
        || shader_frequency == EShaderFrequency::RayMiss as u8
        || shader_frequency == EShaderFrequency::RayHitGroup as u8
        || shader_frequency == EShaderFrequency::RayCallable as u8
}

fn build_io_store_shader_code_archive_header(
    shader_library: &FShaderLibraryHeader,
    shader_format_name: &str,
    container_version: EIoStoreTocVersion,
    max_uncompressed_shader_group_size: usize,
) -> FIoStoreShaderCodeArchiveHeader {
    let mut shader_to_referencing_shader_maps: HashMap<usize, Vec<usize>> =
        HashMap::with_capacity(shader_library.shader_hashes.len());
    ...
    for shader_map_index in 0..shader_library.shader_map_hashes.len() {
        ...
        for i in 0..shader_map_entry.num_shaders {
            ...
            shader_to_referencing_shader_maps
                .entry(shader_index)
                .or_default()
                .push(shader_map_index);
        }
    }
    ...
    let mut sorted_shader_indices: Vec<usize> =
        shader_to_referencing_shader_maps.keys().cloned().collect();
    ...
    for shader_index in sorted_shader_indices {
        let referencing_shader_maps = shader_to_referencing_shader_maps
            .get(&shader_index)
            .unwrap();
    ...
    fn sort_shaders_ascending(
        shader_index_a: usize,
        shader_index_b: usize,
        shader_library: &FShaderLibraryHeader,
    ) -> Ordering {
        ...
        shader_code_entry_a
            ...
            .then(
                shader_code_entry_a
                    .frequency
                    .cmp(&shader_code_entry_b.frequency),
            )
    ...
    shader_groups = shader_groups
        ...
        .flat_map(|x| {
            ...
            for shader_index in &x {
                group_size +=
                    shader_library.shader_entries[*shader_index].uncompressed_size as usize;
            }
            ...
            for shader_index in sorted_shaders {
                ...
                for new_shader_group_index in 1..num_new_groups {
                    if new_shader_group_sizes[new_shader_group_index]
                        < new_shader_group_sizes[smallest_new_group_index]
                    {
    ...
    let mut io_store_library_header = FIoStoreShaderCodeArchiveHeader {
        ...
        shader_entries: vec![
            FIoStoreShaderCodeEntry::default();
            shader_library.shader_hashes.len()
        ],
    ...
    fn find_or_add_sequence_in_shader_indices(
        library_header: &mut FIoStoreShaderCodeArchiveHeader,
        indices: &Vec<usize>,
    ) -> usize {
    ...
    for (shader_group_index, shader_group) in shader_groups.iter().enumerate() {
        ...
        for shader_index in shader_group {
            ...
            io_store_library_header.shader_entries[*shader_index] = FIoStoreShaderCodeEntry::new(
                shader_group_index,
                uncompressed_group_size,
                shader_code_entry.frequency,
            );
        ...
        let group_indices_offset =
            find_or_add_sequence_in_shader_indices(&mut io_store_library_header, shader_group);

        // Prime uncompressed size, but leave compressed size as zero. it will be written later
        io_store_library_header
            .shader_group_entries
            .push(FIoStoreShaderGroupEntry {
        ...
        io_store_library_header.shader_group_chunk_ids.push(
            FIoChunkId::create_shader_code_chunk_id(&shader_hash)
                .with_version(container_version)
                .get_raw(),
        );
    }
    io_store_library_header
...
pub(crate) fn read_shader_asset_info(
    shader_asset_metadata_buffer: &[u8],
    package_name_to_shader_maps: &mut HashMap<String, Vec<FSHAHash>>,
) -> anyhow::Result<()> {
    let shader_asset_info: ShaderAssetInfoFileRoot =
        serde_json::from_slice(shader_asset_metadata_buffer)?;

    for shader_map_entry in &shader_asset_info.shader_code_to_assets {
        for package_name in &shader_map_entry.package_names {
            package_name_to_shader_maps
                .entry(package_name.clone())
                .or_default()
                .push(shader_map_entry.shader_map_hash);
        }
    }
    Ok(())
}

pub(crate) fn write_io_store_library(
    store_writer: &mut IoStoreWriter,
    raw_shader_library_buffer: &Vec<u8>,
    ...
    if shader_library_version_loose != 2 {
        bail!(
            "Unknown shader library file version {}. Current version is 2",
            shader_library_version_loose
        );
    }
    ...
    let total_library_compressed_size =
        raw_shader_library_buffer.len() - shader_library_code_start_offset as usize;

    // Figure out the name of the format of this shader library
    let library_filename = UEPath::new(shader_library_path)
        .file_stem()
        .ok_or_else(|| anyhow!("Failed to retrieve file stem from shader library path"))?;
    let shader_library_name_separator_index = library_filename.find('-').ok_or_else(|| {
        anyhow!("Failed to derive shader library name from shader library filename")
    })?;
    let shader_format_separator_index = library_filename
        .rfind('-')
        .ok_or_else(|| anyhow!("Failed to derive format name from shader library filename"))?;
    ...
    let shader_library_name = library_filename
        [shader_library_name_separator_index + 1..shader_format_separator_index]
        .to_string();
    ...
    let mut io_store_library_header = build_io_store_shader_code_archive_header(
        &shader_library_header,
        &shader_format_name,
        store_writer.container_version(),
        max_shader_group_size,
    );
    ...
    for shader_group_index in 0..io_store_library_header.shader_group_entries.len() {
        ...
        for i in 0..shader_group_entry.num_shaders {
            let shader_indices_index = (shader_group_entry.shader_indices_offset + i) as usize;
            let shader_index =
                io_store_library_header.shader_indices[shader_indices_index] as usize;
            ...
            if shader_code_entry.size != shader_code_entry.uncompressed_size {
                uncompressed_shader_code = decompress_shader_code(
                    &uncompressed_shader_code,
                    &mut compression_method,
                    shader_code_entry.uncompressed_size as usize,
                )?;
            }

            // Write the shader code into the uncompressed chunk buffer. Make sure shader code offset matches it's actual placement
            let expected_shader_code_offset = io_store_library_header.shader_entries[shader_index]
                .shader_uncompressed_offset_in_group();
        ...
        if compression_method.is_some() {
            let compressed_group_data =
                compress_shader(&shader_group_chunk_buffer, compression_method.unwrap())?;
        ...
        io_store_library_header.shader_group_entries[shader_group_index].compressed_size =
            compressed_group_size as u32;
        let shader_group_chunk_id =
            io_store_library_header.shader_group_chunk_ids[shader_group_index];
    ...
    FIoStoreShaderCodeArchiveHeader::serialize(
        &io_store_library_header,
        &mut io_store_shader_library_writer,
        io_store_library_version,
    )?;

    // Write the shader library header chunk using the provided filename
    let shader_library_chunk_id =
        FIoChunkId::create_shader_library_chunk_id(&shader_library_name, &shader_format_name);
    store_writer.write_chunk(
        shader_library_chunk_id,
        Some(shader_library_path),
        &io_store_shader_library_buffer,
    )?;

    // Write statistics
    if log.allow_stdout() {
        let recompression_ratio = f64::round(
            (total_library_compressed_size as f64 / total_compressed_groups_size as f64) * 100.0f64,
        ) as i64;
        let compression_ratio = f64::round(
            (total_library_uncompressed_size as f64 / total_compressed_groups_size as f64)
                * 100.0f64,
        ) as i64;
...
mod test {
    ...
    fn test_read_container_shader_library() -> anyhow::Result<()> {
        ...
        let shader_library_header = ser_hex::read(
            "out/read_container_shader_library.trace.json",
            &mut stream,
            |x| {
                let library_version: u32 = x.de()?;
                assert_eq!(
                    library_version, 1,
                    "expected shader library header version to be initial"
                );
                FIoStoreShaderCodeArchiveHeader::deserialize(
                    x,
                    EIoStoreShaderLibraryVersion::Initial,
                )
            },
        )?;
        ...
        shader_library_header.shader_entries.iter().for_each(|x| {
            ...
            let uncompressed_group_size = shader_library_header.shader_group_entries
                [x.shader_group_index()]
    ...
    fn test_zen_shader_library_identity_conversion() -> anyhow::Result<()> {
        ...
        assert_eq!(
            ...
            "expected legacy shader library header version to be 2"
        );
        let legacy_shader_library =
            FShaderLibraryHeader::deserialize(&mut legacy_shader_library_reader)?;
        ...
        assert_eq!(
            zen_library_version, 1,
            "expected zen shader library header version to be initial"
        );
        let original_zen_shader_library = FIoStoreShaderCodeArchiveHeader::deserialize(
            &mut zen_shader_library_reader,
            EIoStoreShaderLibraryVersion::Initial,
        )?;

        // Check the data that is completely unchanged from legacy to zen lib first
        assert_eq!(
            legacy_shader_library.shader_hashes,
            original_zen_shader_library.shader_hashes
        );
        assert_eq!(
            legacy_shader_library.shader_map_hashes,
            original_zen_shader_library.shader_map_hashes
        );
        assert_eq!(
            legacy_shader_library.shader_indices,
            original_zen_shader_library.shader_indices
        );

        // Convert the legacy shader library back into zen
        let max_shader_group_size = 1024 * 1024;
        let converted_zen_shader_library = build_io_store_shader_code_archive_header(
            &legacy_shader_library,
            "PCD3D_SM6",
            EIoStoreTocVersion::OnDemandMetaData,
            max_shader_group_size,
        );

        // Compare original zen shader library and converted zen shader library
        assert_eq!(
            converted_zen_shader_library.shader_hashes,
            original_zen_shader_library.shader_hashes
        );
        assert_eq!(
            converted_zen_shader_library.shader_map_hashes,
            original_zen_shader_library.shader_map_hashes
        );
        assert_eq!(
            converted_zen_shader_library.shader_indices,
            original_zen_shader_library.shader_indices
        );
        assert_eq!(
            converted_zen_shader_library.shader_entries,
            original_zen_shader_library.shader_entries
        );
        assert_eq!(
            converted_zen_shader_library.shader_map_entries,
            original_zen_shader_library.shader_map_entries
        );
        assert_eq!(
            converted_zen_shader_library.shader_group_chunk_ids.len(),
            original_zen_shader_library.shader_group_chunk_ids.len()
        );
        assert_eq!(
            converted_zen_shader_library.shader_group_entries.len(),
            original_zen_shader_library.shader_group_entries.len()
        );

        for shader_group_index in 0..converted_zen_shader_library.shader_group_entries.len() {
            assert_eq!(
                converted_zen_shader_library.shader_group_entries[shader_group_index].num_shaders,
                original_zen_shader_library.shader_group_entries[shader_group_index].num_shaders
            );
            assert_eq!(
                converted_zen_shader_library.shader_group_entries[shader_group_index]
                    .shader_indices_offset,
                original_zen_shader_library.shader_group_entries[shader_group_index]
                    .shader_indices_offset
            );
            assert_eq!(
                converted_zen_shader_library.shader_group_entries[shader_group_index]
